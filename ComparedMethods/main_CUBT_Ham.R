...
CUBT_Ham_pi[[data_obj_list[i]]] <- pi_Node
CUBT_Ham_times[i] <- end_time[3] - start_time[3]
# save Leaf number, Tree Depth, Avg Leaf Depth
CUBT_Ham_Depth[i,1] <- K
leaf_growth_depths = tree.depth(as.numeric(row.names(my_cubt_join$frame)))[my_cubt_join[["frame"]][,1] == "<leaf>"]
CUBT_Ham_Depth[i,2] <- max(leaf_growth_depths)
num_growth_leaf = length(leaf_growth_depths)
CUBT_Ham_Depth[i,3] <- sum(leaf_growth_depths)/num_growth_leaf
# maximal tree: save Leaf number, Tree Depth, Avg Leaf Depth
CUBT_maxTree_Depth[i,1] <- length(rownames(my_cubt_maximal[["frame"]])[my_cubt_maximal[["frame"]][,1] == "<leaf>"])
leaf_growth_depths = tree.depth(as.numeric(row.names(my_cubt_maximal$frame)))[my_cubt_maximal[["frame"]][,1] == "<leaf>"]
CUBT_maxTree_Depth[i,2] <- max(leaf_growth_depths)
num_growth_leaf = length(leaf_growth_depths)
CUBT_maxTree_Depth[i,3] <- sum(leaf_growth_depths)/num_growth_leaf
}
writeMat(con = "CUBT_Ham_pi.mat", CUBT_Ham_pi = CUBT_Ham_pi)
writeMat(con = "CUBT_Ham_times.mat", CUBT_Ham_times = CUBT_Ham_times)
writeMat(con = "CUBT_Ham_Depth.mat", CUBT_Ham_Depth = CUBT_Ham_Depth)
writeMat(con = "CUBT_maxTree_Depth.mat", CUBT_maxTree_Depth = CUBT_maxTree_Depth)